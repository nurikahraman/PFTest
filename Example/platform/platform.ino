...
#define D6 6
#define D7 7
//MIN and MAX PWM pulse sizes for "TowerPro Micro Servo 9G SG90", they can be found in servo documentation
#define MAX 2400  
#define MIN 500
...
void getrxp(float pe[]) __attribute__((__optimize__ ("O0")));
...
STOPPRINTPOS 52		//4
SWITCHIRDA 53		//5
SETPOSITIONSINMS 54	//6
SWITCHIRDAOFF 55	//7
GEPOSITION 56		//8
SERVOZEROPOS 57		//9
*/
Serial.println("--------  ANA MENU  -------");
	Serial.println("");
	Serial.println("0. LCD arka aydinlatmayi KAPAT");
	Serial.println("1. LCD arka aydinlatmayi AC");
	Serial.println("2. Elle konum girisi");
	Serial.println("3. LCD uzerinde mevcut degerleri goruntule");
	Serial.println("4. LCD uzerinde mevcut degerleri goruntulemeyi durdur");
	Serial.println("5. Uzaktan kumanda olayi, kullanmiyoruz bunu, yok yani:)");
	Serial.println("6. Girilen degerleri aninda servolara uygula");
	Serial.println("7. Buda Uzaktan kumanda olayi, kullanmiyoruz bunu, yok yani:)");
	Serial.println("8. Pozisyonu al");
	Serial.println("9. Servo konumlarini sifirla");
	lcd.clear();
	lcd.print("6DOF Platform");
...
//LCD inicialisation and turning on the backlight
   ...
   lcd.home();
   lcd.print("6DOF Platform");
   	//Serial.write('SETUP Yordaýmina girildi');
...
void loop()
{
	...
	//lcd.print('Loop Yordami');

	//Serial.write('Loop Yordaýmina girildi');
...
//action to change position of platform, obtain 6 values representing desired position
         case SETPOSITIONS:
			 Serial.println("--------  POZISYON AYARLAMA YORDAMI  -------");
			 lcd.clear();
			 lcd.print("2.Konum Ayarla.");
            for(int i=0;i<6;i++){
			   Serial.print("Okuma icin bekleyen Seri Veri Miktari: ");
			   Serial.println(Serial.available());
			   Serial.print("Mevcut okuma bekleyen deger: ");
			   Serial.println(Serial.read());


			   Serial.print(i);
			   Serial.println(". Degeri gir: ");
               long kk;
			   ...
			   Serial.print(" -   Algilanan Deger(ASCI Karsiligi): ");
			   Serial.println(Serial.read());
			   kk=(long)Serial.read();
			   Serial.print(" -   Girilen Degerin (long) tipi: ");
			   Serial.println(kk);

               kk=kk+(Serial.read()<<8);
               kk=kk+(Serial.read()<<16);
               kk=kk+(Serial.read()<<24);
			   Serial.print(" -   Girilen degerin son durumu: ");
			   Serial.println(kk);
			   Serial.println("");
			   Serial.println("");
			   Serial.println("");
...
//main control would be executed on communicating partner
         case SETPOSITIONSINMS:
			 Serial.println("--------  MOTORLARI TEK TEK KONUMLANDIRMA  -------");
			 lcd.clear();
			 lcd.print("6.Konum Ayarla");
			 for (int i = 0;i<6;i++) {
				 Serial.print("Okuma icin bekleyen Seri Veri Miktari: ");
				 Serial.println(Serial.available());
				 Serial.print("Mevcut okuma bekleyen deger: ");
